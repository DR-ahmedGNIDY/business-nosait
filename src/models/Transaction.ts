...
import { PAYMENT_METHOD, TRANSACTION_STATUS, TRANSACTION_TYPE, TRANSACTION_SOURCE } from "@/lib/constants";
...
export interface ITransaction {
  ...
  source: (typeof TRANSACTION_SOURCE)[number];
...
const TransactionSchema = new Schema<ITransaction>(
  {
    ...
    type: { type: String, enum: TRANSACTION_TYPE, default: "income" },
    method: { type: String, enum: PAYMENT_METHOD, default: "cash" },
    status: { type: String, enum: TRANSACTION_STATUS, default: "completed" },
    source: { type: String, enum: TRANSACTION_SOURCE, default: "project" },
...
TransactionSchema.index({ status: 1, source: 1, deletedAt: 1 });
